...
const char *s3c_ts_name = "Goodix_TouchScreen";
...
static int  goodix_read_version(struct goodix_ts_data *ts)
{
	...
	//hcj: for debug
	printk(KERN_INFO " Guitar Version: %s\n", &version_data[1]);
...
BIT_NO_CHANGE:
	...
	if(finger_list.length > 0 && finger_list.pointer[0].state == FLAG_DOWN)
	{
		input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (800 - pos[0][0]));
	...
	for(count = 1; count < MAX_FINGER_NUM; count++)
	{
		if (finger_list.length > count)
		{
			...
			{
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (800 -pos[count][0]));
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* hcj: for debug guitar */
	printk(KERN_INFO "Install touchscreen driver for guitar.\n");
	//Check I2C function
	...
	/* hcj: for debug guitar */
	printk(KERN_INFO "Registered input device for guitar.\n");
	...
	if (client->irq)
	{
		...
		{	
			...
			//hcj: debug 
            printk(KERN_CRIT "Request EIRQ %d succesd on GPIO:%d\n",TS_INT,INT_PORT);
		}
	}

	/* hcj: for debug guitar */
	printk(KERN_INFO "End of probe for guitar.\n");
